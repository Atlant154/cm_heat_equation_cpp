...
inline double_t HeatSources(double_t const x, double_t const t) {
    return std::pow(x, 2.) * (std::pow(x, 2.) - 12 * diffusivity_coefficient * t) + std::exp(x) * (x * t *
...
TEST(Accuracy, LargeMesh) {
    ...
    double_t const second_error{HeatEquationApprError(h_num * factor, tau_num * factor * factor)};
