    ...
    , matrix_main_(1. + ((2. * a_ * tau_) / std::pow(h_, 2.)))
{
    ...
    for (uint32_t time_iter {1}; time_iter < time_layers_num_; ++time_iter) {
        for (uint32_t space_iter {1}; space_iter < h_num_ - 1; ++space_iter)
            ta_free_part.emplace_back(time_layer[space_iter] + tau_
                         * heat_sources(x_left_bound_ + space_iter * h_, time_left_bound_ + time_iter * tau_));
        ...
        std::copy(std::begin(ta_result), std::end(ta_result), std::back_inserter(time_layer));
